function [x,histout,costdata] = projbfgs(x0,f,fgrad,up,low,tol,maxit)
%
...
for i=1:ndim
	kku(i)=up(i);
	kkl(i)=low(i);
	if kkl(i) > kku(i), error('lower bound exceeds upper bound'), end
end
...
alp=1.d-4;
nsmax=5;
ystore=zeros(ndim,nsmax);
sstore=ystore;
ns=0;

if nargin < 6, maxit=1000; end
if nargin < 5, tol=1.d-6; end

itc=1;
fc=feval(f,xc,'fun'); numf=1;
gc=feval(fgrad,xc); numg=1;
...
for i=1:ndim;
	if( min(kku(i)-xc(i),xc(i)-kkl(i)) < epsilon), alist(i)=1; end;
end;
...
while(norm(pgc) > tol & itc <= maxit)
	...
	fc=feval(f,xt,'fun'); numf=numf+1;
	gp=feval(fgrad,xt); numg=numg+1;
	y=gp-gc; s=xt-xc;
